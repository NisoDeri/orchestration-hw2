...
def _reply(args: list[dict], idx: int) -> DebaterReply:
    d = args[idx % len(args)]
    return DebaterReply(
        argument=d["arg"], confidence=d["conf"],
        attack_points=d["attack"], defense_points=d["defense"],
        citations=[d["cite"]],
...
class DemoJudge:
    ...
    def verdict(self, sb: ScoreAggregate) -> Verdict:
        return Verdict(
            winner="debater-a", score_a=sb.total_a() + 2, score_b=sb.total_b(),
            reasoning=(
            ...
            rubric_breakdown={
                "debater-a": RubricBreakdown(
                    logic=sb.a_logic, evidence=sb.a_evidence,
                    persuasion=sb.a_persuasion, counter=sb.a_counter,
                ),
                "debater-b": RubricBreakdown(
                    logic=sb.b_logic, evidence=sb.b_evidence,
