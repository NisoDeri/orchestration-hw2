...
def _attach_ui_support(emitter: EventEmitter) -> None:
    ...
    def _on_event(event: Any) -> None:
        ...
        if idx < len(CROWD_REACTIONS):
            ...
            emitter.emit(
                "crowd_reaction", {"emojis": emojis, "one_liner": f"{liner} (leaning {d})"}
            )
            time.sleep(0.2)
        if idx < len(FACT_CHECKS) and FACT_CHECKS[idx]:
            claims = [
                {"quote": fc["claim"], "verdict": fc["verdict"], "note": fc["note"]}
