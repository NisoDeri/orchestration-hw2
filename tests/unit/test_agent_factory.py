...
def _models_cfg() -> ModelsConfig:
    ...
    return ModelsConfig(
        ...
        default_betas=["skills-2025-10-02"], agents=agents,
    )
...
def _persona(name: str) -> Persona:
    return Persona.model_validate({
        ...
        "color_hex": "#123456",
        "system_prompt": f"You argue for {name}.", "style_notes": ["passionate"],
...
def test_all_agents_created() -> None:
    agents = create_agents(
        setup=_setup(), models=_models_cfg(),
        persona_a=_persona("Messi"), persona_b=_persona("Ronaldo"),
        facts_service=_facts_service(), gatekeeper=MagicMock(spec=Gatekeeper),
...
def test_optional_agents_skipped_when_disabled() -> None:
    ...
    agents = create_agents(
        setup=_setup(enabled), models=_models_cfg(),
        persona_a=_persona("Messi"), persona_b=_persona("Ronaldo"),
        facts_service=_facts_service(), gatekeeper=MagicMock(spec=Gatekeeper),
        memory=ConversationMemory("d1", "Messi vs Ronaldo"),
        context_builder=ContextBuilder(),
