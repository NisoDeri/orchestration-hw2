...
_SETUP = {
    ...
    "agents_enabled": {
        "judge": True, "debater_a": True, "debater_b": True,
...
def test_models_config_agent_temperature_range() -> None:
    base = {
        ...
        "agents": {
            "judge": {
                "model": "claude-opus-4-7", "temperature": 0.5, "max_tokens": 1000,
                "skill_id": "x", "tools": [], "tool_choice": {"type": "auto"},
                "cache_system": True,
            }
        },
    }
...
def test_rate_limits_config_loads() -> None:
    RateLimitsConfig.model_validate({
        ...
        "anthropic": {"max_requests_per_second": 10.0, "max_concurrent": 4,
                      "queue_max_depth": 64, "queue_max_block_s": 30.0},
        "retry_policy": {"max_retries": 3, "backoff_base_s": 0.5,
                         "backoff_factor": 2.0, "backoff_max_s": 8.0,
...
def test_logging_config_loads() -> None:
    LoggingConfig.model_validate({
        "version": "1.00",
        "dir": "logs", "max_files": 20, "lines_per_file": 500,
        "default_level": "INFO", "level_overrides": {}, "tail_default_n": 100,
    })
...
def test_facts_config_loads() -> None:
    cfg = FactsConfig.model_validate({
        ...
        "claims": {"ballon_messi": {"value": 8, "as_of": "2024",
                                    "source": "https://x.com", "severity_on_mismatch": 0.6}},
        "patterns": [],
    })
...
def test_agent_model_config_max_tokens_positive() -> None:
    with pytest.raises(ValidationError):
        AgentModelConfig.model_validate({
            "model": "x", "temperature": 0.5, "max_tokens": 0, "skill_id": "x",
            "tools": [], "tool_choice": {"type": "auto"}, "cache_system": False,
        })
