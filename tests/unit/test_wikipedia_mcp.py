...
def test_summary_returns_result_field_on_success() -> None:
    ...
    with (
        patch("debate_ai.tools.wikipedia_mcp.shutil.which", return_value="/usr/bin/uvx"),
...
def test_search_returns_list_when_provided_directly() -> None:
    ...
    with (
        patch("debate_ai.tools.wikipedia_mcp.shutil.which", return_value="/usr/bin/uvx"),
        patch("debate_ai.tools.wikipedia_mcp.subprocess.run", return_value=MagicMock(stdout=fake)),
    ):
